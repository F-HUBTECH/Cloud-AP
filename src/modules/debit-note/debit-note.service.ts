...
class DebitNoteService {
  ...
  async list(params: DebitNoteListParams = {}): Promise<DebitNoteListResult> {
    ...
    return {
      data: (data ?? []).map(this.mapDebitNote),
      total: count ?? 0,
      page,
      pageSize,
    };
  ...
  async getById(id: string): Promise<DebitNoteWithVendor> {
    const supabase = await this.getClient();
    ...
    if (headerError || !header) {
      throw new NotFoundError("DebitNote", id);
    }
    ...
    return {
      ...this.mapDebitNote(header),
      supplier: vendor
        ? { code: vendor.code, name: vendor.name_en, name_th: vendor.name_th, tax_id: vendor.tax_id }
        : { code: header.supplier_code, name: "", name_th: null, tax_id: null },
      items: (items ?? []).map(this.mapDebitNoteItem),
    };
  }

  async create(formData: DebitNoteFormData): Promise<DebitNote> {
    if (!(await canCreate(MODULE_CODES.VOUCHER_AP))) {
      throw new AppError("No permission to create debit note", "FORBIDDEN", 403);
    }
    ...
    try {
      ...
      await glPostingService.createJournalEntry({
        ...
        description: `Debit Note ${docNumber} - ${formData.supplierCode}`,
        lines: [
          ...
          {
            glAccount: tradeGl,
            description: `AP Trade DR - ${docNumber}`,
            debit: totals.totalApTrade,
            credit: 0,
          },
        ],
        createdBy: null,
      ...
      // GL posting failure should not block debit note creation
    }
    ...
    await logAudit({
      ...
      recordId: headerId,
      action: "create",
      newData: { docNumber, supplierCode: formData.supplierCode, amount: formData.amount, apType: AP_TYPE_DR },
      detail: `Created debit note ${docNumber}`,
    });

    return this.mapDebitNote(header);
  }

  async update(id: string, formData: Partial<DebitNoteFormData>): Promise<DebitNote> {
    if (!(await canUpdate(MODULE_CODES.VOUCHER_AP))) {
      throw new AppError("No permission to update debit note", "FORBIDDEN", 403);
    }
  ...
  async delete(id: string): Promise<void> {
    if (!(await canDelete(MODULE_CODES.VOUCHER_AP))) {
      throw new AppError("No permission to delete debit note", "FORBIDDEN", 403);
    }
    ...
    await logAudit({
      ...
      recordId: id,
      action: "delete",
      oldData: { docNumber: existing.doc_number, amount: existing.total_amount, apType: AP_TYPE_DR },
      detail: `Hard deleted debit note ${existing.doc_number}`,
    });
  }

  async cancel(id: string, reason: string): Promise<DebitNote> {
    if (!(await canUpdate(MODULE_CODES.VOUCHER_AP))) {
      throw new AppError("No permission to cancel debit note", "FORBIDDEN", 403);
    }
    ...
    if (["cancelled", "voided"].includes(existing.status as string)) {
      throw new AppError("Debit note is already cancelled or voided", "ALREADY_CANCELLED", 422);
    }

    if (existing.status === "paid") {
      throw new AppError("Cannot cancel a paid debit note", "DR_PAID", 422);
    }
    ...
    try {
      ...
      // GL cancellation failure should not block cancel
    }
  ...
  private mapDebitNoteItem(row: Record<string, unknown>): DebitNoteItem {
    return {
      id: row.id as string,
