...
import { createDebitNote } from "@/modules/debit-note/debit-note.actions";
...
export default function NewDebitNotePage() {
  ...
  useEffect(() => {
    async function fetchVendors() {
      ...
      const { data } = await supabase
        .from("vendors")
        .select("id, code, name_en, name_th, vat_code, wht_code, credit_term")
        .eq("is_active", true)
        .order("code");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const docDate = new Date(header.doc_date);
      const periodMonth = String(docDate.getMonth() + 1).padStart(2, "0");
      const periodYear = String(docDate.getFullYear());

      const formData = {
        documentDate: header.doc_date,
        supplierCode: selectedVendor.code,
        supplierId: selectedVendor.id,
        invoiceNumber: header.inv_number || "",
        amount: totalDr,
        debit: totalDr,
        credit: totalCr,
        vatType: header.vat_type as "none" | "exclusive" | "inclusive" | "exempt",
        creditTerm: selectedVendor.credit_term,
        dueDate: header.doc_date,
    ...
    <div className="space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex justify-end gap-3">
          <Link href="/debit-notes" className="btn-outline">
            Cancel
          </Link>
